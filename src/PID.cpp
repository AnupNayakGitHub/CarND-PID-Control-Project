...
PID::PID():p_error(0.0), i_error(0.0), d_error(0.0),
           Kp(0.0), Ki(0.0), Kd(0.0), tune(false) {}
...
bool PID::IsTuning() {
  return (tune && ((!twdlr.isStable()) || (twdlr_st != 2)));
}
...
void PID::UpdateError(double cte, double speed) {
  ...
  cout << "Ks  [" << Kp << ", " << Ki <<", " << Kd << "], twdlr_st " << twdlr_st << endl;
  cout << "Errors  [" << p_error << ", " << i_error <<", " << d_error << "], twdlr_st " << twdlr_st << endl;
  d_error = cte - p_error;
  p_error = cte;
  i_error += (cte);
  this->speed = speed;
